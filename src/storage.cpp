...
StorageHandler::StorageHandler(const std::string &_walletFile,
  ...
  loadData();
  // TODO: loadAllIndexes, if running in gui mode
}
...
int StorageHandler::getTransactionsByCategory(
    const std::string &category, std::vector<Transaction> &result) {
  populateCategoryIdx();
 ...
 */
int StorageHandler::retrieveDailyExpenses(const std::string &base_date,
                                          std::vector<Transaction> &result) {
  populateDateHash();
 ...
 * @return int -1 on empty, 0 on success
 */
int StorageHandler::retrieveWeeklyExpenses(const std::string &base_date,
                                           std::vector<Transaction> &result) {
  populateDateMap();
 ...
 * @return int -1 on empty, 0 on success
 */
int StorageHandler::retrieveMonthlyExpenses(const std::string &base_date,
                                            std::vector<Transaction> &result) {
  populateDateMap();
...
/**
 * @brief Wrapper function that calls the specific range function,
 ...
 * @return int 
 */
int StorageHandler::retrieveExpenses(const std::string &base_date, int range,
                                     std::vector<Transaction> &result) {
  switch (range) {
  case 1:
...
// TODO: this function has to be redone with the indexing system
